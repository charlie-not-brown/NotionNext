import BLOG from '@/blog.config'
import { siteConfig } from '../../config'

/**
 * Notion 自定义 Emoji 映射表
 *
 * 左边：Notion Custom Emoji ID
 * 右边：本站本地图片路径
 */
const CUSTOM_EMOJI_MAP = {
  '2c9ff97e-5407-8025-9136-007a7c20940b':
    '/images/notion-icons/Batman-Logo-2000-2011.webp',

  '2c9ff97e-5407-80f8-b7b0-007a6f023039':
    '/images/notion-icons/cassandra.webp',

  '2c9ff97e-5407-80d0-ad42-007a33a9fa94':
    '/images/notion-icons/nightwing.webp',

  '2c8ff97e-5407-80f0-8662-007a9845c472':
    '/images/notion-icons/batman1.webp',

  '2c9ff97e-5407-80e6-b0c3-007adaecfbdd':
    '/images/notion-icons/Detective-Comics.webp',

  '2c8ff97e-5407-80bd-8830-007a8ec587ab':
    '/images/notion-icons/robin-timdrake.webp',

  '2c8ff97e-5407-80ae-be28-007af2cfeac4':
    '/images/notion-icons/red-robin.webp',

  '2caff97e-5407-8031-9797-007a0febd278':
    '/images/notion-icons/Teen-Titans.webp',

  '2c9ff97e-5407-803d-a9ed-007ad2083d1':
    '/images/notion-icons/Teen-Titans2.webp',

  '2c8ff97e-5407-8028-91ec-007a048af5f8':
    '/images/notion-icons/yj.webp',

  '2caff97e-5407-8072-b1a0-007aa8e9f049':
    '/images/notion-icons/yj-v3.webp'

  // 后续可补充
}

/**
 * 图片映射
 *
 * @param {*} img 图片地址，可能是相对路径，可能是外链
 * @param {*} block 数据块，可能是单个内容块，可能是Page
 * @param {*} type block 单个内容块 ； collection 集合列表
 * @param {*} from 来自
 * @returns
 */
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  if (!img) {
    return null
  }

  // 相对目录，则视为notion的自带图片
  if (img.startsWith('/')) {
    ret = BLOG.NOTION_HOST + img
  } else {
    ret = img
  }

  /**
   * 修复 Notion 自定义 Emoji
   *
   * 原始格式：
   * notion://custom_emoji/space-id/emoji-id?t=xxx
   *
   * 转换为：
   * /images/notion-icons/xxx.png
   */
  if (
    typeof img === 'string' &&
    img.startsWith('notion://custom_emoji/')
  ) {
    const match = img.match(
      /^notion:\/\/custom_emoji\/[^/]+\/([^?]+)/
    )

    const emojiId = match?.[1]

    if (emojiId && CUSTOM_EMOJI_MAP[emojiId]) {
      return CUSTOM_EMOJI_MAP[emojiId]
    }
  }

  let ret = null

  /**
   * 修复 Notion 自定义 Emoji / 页面 Icon 图片
   *
   * Notion 自定义 Emoji 通常返回这种地址：
   * https://app.notion.com/image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2F...
   *   ?table=custom_emoji
   *   &imgBuildSrc=CustomEmoji
   *
   * 公开前端直接请求 app.notion.com/image 可能出现 401 或图裂，
   * 因此提取 URL 中已经包含的原始图片地址，直接加载原图。
   */
  if (
    ret.startsWith('https://app.notion.com/image/') &&
    (
      ret.includes('table=custom_emoji') ||
      ret.includes('imgBuildSrc=CustomEmoji')
    )
  ) {
    try {
      const notionImageUrl = new URL(ret)

      // pathname 形式：
      // /image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2F...
      const encodedOriginalUrl = notionImageUrl.pathname.replace(
        /^\/image\//,
        ''
     )

      const originalUrl = decodeURIComponent(encodedOriginalUrl)

      // 只接受正常的 http / https 图片地址，避免意外替换
      if (
        originalUrl.startsWith('https://') ||
        originalUrl.startsWith('http://')
      ) {
        ret = originalUrl
      }
    } catch (error) {
      console.warn('Failed to decode Notion custom emoji URL:', ret, error)
    }
  }

  const hasConverted =
    ret.indexOf('https://www.notion.so/image') === 0 ||
    ret.includes('notion.site/images/page-cover/')


  // 需要转化的URL ; 识别aws图床地址，或者bookmark类型的外链图片
  // Notion新图床资源 格式为 attachment:${id}:${name}
  const needConvert =
    !hasConverted &&
    (block.type === 'bookmark' ||
      ret.includes('secure.notion-static.com') ||
      ret.includes('prod-files-secure')) ||
      ret.indexOf('attachment')===0


  // Notion旧图床
  if (needConvert) {
    ret =
      BLOG.NOTION_HOST +
      '/image/' +
      encodeURIComponent(ret) +
      '?table=' +
      type +
      '&id=' +
      block.id
  }

  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    if (BLOG.RANDOM_IMAGE_URL) {
      // 只有配置了随机图片接口，才会替换图片
      const texts = BLOG.RANDOM_IMAGE_REPLACE_TEXT
      let isReplace = false
      if (texts) {
        const textArr = texts.split(',')
        // 判断是否包含替换的文本
        textArr.forEach(text => {
          if (ret.indexOf(text) > -1) {
            isReplace = true
          }
        })
      } else {
        isReplace = true
      }
      if (isReplace) {
        ret = BLOG.RANDOM_IMAGE_URL
      }
    }

    // 图片url优化，确保每一篇文章的图片url唯一
    if (
      ret &&
      ret.length > 4 &&
      !ret.includes('https://www.notion.so/images/')
    ) {
      // 图片接口拼接唯一识别参数，防止请求的图片被缓，而导致随机结果相同
      const separator = ret.includes('?') ? '&' : '?'
      ret = `${ret.trim()}${separator}t=${block.id}`
    }
  }

  // 统一压缩图片
  if (needCompress) {
    const width = block?.format?.block_width
    ret = compressImage(ret, width)
  }

  return ret
}

/**
 * 是否是emoji图标
 * @param {*} str
 * @returns
 */
function isEmoji(str) {
  const emojiRegex =
    /[\u{1F300}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F018}-\u{1F270}\u{238C}\u{2B06}\u{2B07}\u{2B05}\u{27A1}\u{2194}-\u{2199}\u{2194}\u{21A9}\u{21AA}\u{2934}\u{2935}\u{25AA}\u{25AB}\u{25FE}\u{25FD}\u{25FB}\u{25FC}\u{25B6}\u{25C0}\u{1F200}-\u{1F251}]/u
  return emojiRegex.test(str)
}

/**
 * 压缩图片
 * 1. Notion图床可以通过指定url-query参数来压缩裁剪图片 例如 ?xx=xx&width=400
 * 2. UnPlash 图片可以通过api q=50 控制压缩质量 width=400 控制图片尺寸
 * @param {*} image
 */
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  if (!image || image.indexOf('http') !== 0) {
    return image
  }

  if (image.includes(".svg")) return image

  if (!width || width === 0) {
    width = siteConfig('IMAGE_COMPRESS_WIDTH')
  }


  let urlObj
  let params
  try {
    urlObj = new URL(image)
    params = new URLSearchParams(urlObj.search)
  } catch (err) {
    // 如果解析失败，尝试 decodeURIComponent 再解析
    try {
      const decoded = decodeURIComponent(image)
      urlObj = new URL(decoded)
      params = new URLSearchParams(urlObj.search)
    } catch (e) {
      console.error('compressImage: Invalid URL:', image, err)
      return image
    }
  }
  
  // Notion图床
  if (
    image.indexOf(BLOG.NOTION_HOST) === 0 &&
    image.indexOf('amazonaws.com') > 0
  ) {
    params.set('width', width)
    params.set('cache', 'v2')
    // 生成新的URL
    urlObj.search = params.toString()
    return urlObj.toString()
  } else if (image.indexOf('https://images.unsplash.com/') === 0) {
    // 压缩unsplash图片
    // 将q参数的值替换
    params.set('q', quality)
    // 尺寸
    params.set('width', width)
    // 格式
    params.set('fmt', fmt)
    params.set('fm', fmt)
    // 生成新的URL
    urlObj.search = params.toString()
    return urlObj.toString()
  } else if (image.indexOf('https://your_picture_bed') === 0) {
    // 此处还可以添加您的自定义图传的封面图压缩参数。
    // .e.g
    return 'do_somethin_here'
  }

  return image
}

export { compressImage, mapImgUrl }
